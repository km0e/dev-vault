...
impl Target {
    ...
    pub fn get_dst_uid(&self) -> Result<&str> {
        self.dst_uid
            .as_deref()
            .with_whatever_context::<_, &str, Error>(|| "no dst_uid")
    }
    pub fn get_uid(&self) -> Result<(&str, &str)> {
        Ok((
            self.src_uid
                .as_deref()
                .with_whatever_context::<_, &str, Error>(|| "no src_uid")?,
            self.get_dst_uid()?,
        ))
    }
    pub fn filter(&self, filter: &mut UserFilter) {
