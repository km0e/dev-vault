...
impl AutoX {
    pub async fn new(_: bool) -> windows::core::Result<Self> {
        info!("AutoX::new");
        let sch_scmanager =
    ...
    pub async fn reload(&self, name: impl AsRef<OsStr>) -> windows::core::Result<()> {
        let name = name.as_ref();
        let sch_service = ServiceControlHandle::new(
            self.sch_scmanager,
            name,
